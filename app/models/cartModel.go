...
func (c *Cart) GetCart(db *gorm.DB, cartID string) (*Cart, error) {
	...
	err = db.Preload("CartItems").Preload("CartItems.Product").Model(Cart{}).Where("id = ?", cartID).First(&cart).Error
	if err != nil {
		return nil, err
	}
...
func (c *Cart) CalculateCart(db *gorm.DB, cartID string) (*Cart, error) {
	...
	//err := db.First(&cart, "id = ?", c.ID).Updates(updateCart).Error
	err := db.Model(Cart{}).Where("id = ?", c.ID).Updates(updateCart).Error
	if err != nil {
		return nil, err
	}
...
func (c *Cart) AddItem(db *gorm.DB, item CartItem) (*CartItem, error) {
	...
	err = db.Model(CartItem{}).Where("cart_id = ?", c.ID).Where("product_id = ?", product.ID).First(&existItem).Error
	if err != nil {
