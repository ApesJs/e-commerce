...
func (p *Product) GetProducts(db *gorm.DB, perPage, page int) (*[]Product, int64, error) {
	var err error
	...
	err = db.Model(&Product{}).Order("created_at desc").Limit(perPage).Offset(offset).Find(&products).Error
	if err != nil {
		return nil, 0, err
	}
...
func (p *Product) FindBySlug(db *gorm.DB, slug string) (*Product, error) {
	...
	err = db.Preload("ProductImages").Model(&Product{}).Where("slug = ?", slug).First(&product).Error
	if err != nil {
		return nil, err
	}
...
func (p *Product) FindByID(db *gorm.DB, productID string) (*Product, error) {
	...
	err = db.Preload("ProductImages").Model(&Product{}).Where("id = ?", productID).First(&product).Error
	if err != nil {
		return nil, err
	}
